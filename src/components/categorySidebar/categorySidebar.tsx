...
const brands: Brand[] = [
  { name: 'Exemplo Marca 1', isChecked: true },
  { name: 'Exemplo Marca 2' },
  { name: 'Exemplo Marca 3' },
  { name: 'Exemplo Marca 4' },
  { name: 'Exemplo Marca 5' },
];

const SectionTitleAtom: React.FC<{ title: string }> = ({ title }) => {
  return <h3 className="text-primary font-primary font-bold text-md py-4 pl-4 uppercase border border-b-0">{title}</h3>;
};

const CategoryListAtom: React.FC<{ categories: Category[] }> = ({ categories }) => {
  return (
          ...
          {category.subCategories && (
            <ul className="ml-4 space-y-1">
              {category.subCategories.map((sub, index) => (
                <li key={index}>
                  <a href="#" className="text-gray-500 text-xs hover:text-primary py-1 block">
                    {sub}
                  </a>
                </li>
              ))}
